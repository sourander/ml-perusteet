...
Kämäräisen käyttämä suomennos klusteroinnista on *ryvästys* [^kämäräinen]. Hän myös esittää ajatuksen, että useimmat ryvästysmenetelmät ovat vuosikymmeniä vanhoja, mutta niitä kehitetään yhä.
...
Aiemmin esiteltyä k-NN:ää ja nyt käsiteltävää k-means algoritmia yhdistää kirjain `k`, mutta niiden merkitys on eri. k-Meansin kohdalla `k` tarkoittaa `n_clusters` eli ryppäiden määrää, kun taas k-NN:ssä `k` tarkoittaa naapureiden määrää. Toinen merkittävä ero algoritmien välillä on, että k-means ei ole luokittelualgoritmi, vaan – kuten otsikosta varmaan arvaatkin – rypästysmenetelmä. Kyseessä on siis ==ohjaamaton oppiminen== (*engl. unsupervised learning*), mikä tarkoittaa, että opetusnäytteillä ei ole ennalta tiedettyjä luokkia [^kämäräinen]. Nämä ovat siinä mielessä tärkeitä algoritmeja, että maailmassa on tarjolla hyvin pieni selkeästi annotoitu määrä dataa suhteessa täysin kuratoimattomaan dataan. Kämäräinen mainitsee myös SOM:n (Self-Organizing Map), joka on maailmalla tunnetun Teuvo Kohosen kehittämä 1980-luvulla. Tämäkin kehityskulku yhä jatkuu, ja tuoreehkoja käänteitä esitellään katsausartikkelissa *A Survey on Recent Advances in Self-Organizing Maps* [^som-survey]. Me pitäydymme kuitenkin tällä kurssilla ryvästysmenetelmissä ja sivuutamme muut ohjaamattoman oppimisen menetelmät.

Ryvästysmenetelmät voi jakaa ainakin kahteen perheeseen: neliövirhemenetelmiin ja hierarkisiin menetelmiin [^kämäräinen]. Suomalaisten termien sijasta jaan tässä materiaalissa ne vastaavasti seuraaviin kolmeen perheeseen, mutta olisi mahdollista jakaa se myös muihin (esim. grid-based) [^survey-partitional-hierarchical]: 

* Partition-based clustering: [k-Means](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.KMeans.html), [MeanShift](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.MeanShift.html)
* Hierarchical:
    ...
    * Divisive Clustering: [MST](https://pypi.org/project/mistree/)
    * Other: [SOM](https://github.com/JustGlowing/minisom/), [CHAMELEON](https://github.com/JaimeRamos99/chameleon-clustering-for-python)
* Density-based: [DBSCAN](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.DBSCAN.html), [OPTICS](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.OPTICS.html)

## K-Means vaiheet


K-means pyrkii minimoimaan klusterien sisäisen hajonnan eli pisteiden etäisyydet
omaan klusterikeskukseen. K-Means on lähes yhtä simppeli algoritmi kuin k-NN. Aivan kuten k-NN, myös k-meansiä voi käyttää n-ulotteisessa avaruudessa, mutta visualisoinnin vuoksi käytämme tässä esimerkissä vain 2-ulotteista avaruutta. Koulutuksen vaiheet voidaan tiivistää seuraavasti [^fromscratch]:

1. Valitse `k` klusteria, joihin `m` näytettä klusteroidaan.
2. Päätä tai arvo (x,y)-koordinaatit `k` klusterikeskuksille.
3. Nimitä kukin piste kuuluvaksi lähimpään klusteriin ($m$ -> $m_k$)
4. Laske keskiarvo (x,y) jokaiselle $m_k$ havainnolle. Siirrä klusterikeskukset näihin (x,y)-koordinaatteihin.
...
![k-means cluster centrals randomized](../images/510_kmeans_01_starting_points.png)
...
!!! tip
    ...
    Mitäkö scikit-learn käyttää? Katso [KMeans](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.KMeans.html)-dokumentaatiosta.
...
![k-means cluster centrals after first iteration](../images/510_knmeans_02_first_moves.png)
...
![k-means phase by phase](../images/510_kmeans_03_all_iterations.png)
...
!!! tip
    ...
    Tutustu myös muihin klusterointialgoritmeihin, kuten `DBSCAN`, vähintään pintaraapaisuna. Hyvä paikka aloittaa on [Scikit-Learn: Clustering](https://scikit-learn.org/stable/modules/clustering.html). Tarkkasilmäinen opiskelija löytää myös kurssikirjasta esimerkkejä toisen klusterointialgoritmin käytöstä.
...
![k-means original vs predicted](../images/510_kmeans_04_diff.png)
...
!!! warning
    ...
    * Me olisimme voineet päättää segmentoida meidän pisteet kolmen klusterin sijasta kahteen, neljään tai johonkin muuhun `k=n`-määrään. Nämä eivät olisi sen enempää oikeita tai vääriä vastauksia; me saisimme vain ryhmiä, joissa on enemmän tai vähemmän varianssia.
...
xychart
    ...
    line [2400, 980, 380, 300, 260, 195, 182, 175, 165]
```

Elbow-metodi on yksinkertainen. Aja k-means eri $k$ arvoilla, ynnää kaikkien pisteiden etäisyyden klusterikeskukseen, ja piirrä näistä kuvaaja. Yllä olevan kuvaajan kohdalla kuvaajan "kyynärpää" – tai jääkiekkomailan mutka – on noin luvun 4 kohdalla. [^fromscratch] [^geronpytorch] Y-akselista voi käyttää myös nimitystä *inertia*.
...
1. $a(x)$: pisteen keskimääräinen etäisyys oman klusterin muihin pisteisiin  
2. $b(x)$: pisteen keskimääräinen etäisyys lähimmän muun klusterin pisteisiin

Jos käytämme euklidista etäisyyttä, siluettiarvo pisteelle $x$ on [^ml-algorithms]
...
[^som-survey]: Guérin, A., Chauvet, P. & Saubion, F. *A Survey on Recent Advances in Self-Organizing Maps*. https://arxiv.org/abs/2501.08416
[^survey-partitional-hierarchical]: Reddy, C. K., Vinzamuri, B. & Reddy, C. K. *A Survey of Partitional and Hierarchical Clustering Algorithms*. CRC Press. 2014. https://doi.org/10.1201/9781315373515-4
...
[^ml-algorithms]: Bonaccorso, G. *Machine Learning Algorithms, 2nd edition*. Packt Publishing. 2018.
